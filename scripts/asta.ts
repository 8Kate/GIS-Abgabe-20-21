...
async function initAsta(): Promise<void> {
    let response: Response = await fetch("https://astaverleih.herokuapp.com//astaverleih.herokuapp.com/items");
...
function statusAendern(this: HTMLButtonElement): void {
    ...
    let url: string = `https://astaverleih.herokuapp.com//astaverleih.herokuapp.com/statusAendern/${item._id}`;
    console.log(url);
    fetch(url);

...
async function getItem(_id: string): Promise<Item> {
    let response: Response = await fetch("http://astaverleih.herokuapp.com/item/id/" + _id);
...
async function sendReservierung(): Promise<void> {
    ...
    await fetch("http://astaverleih.herokuapp.com/reservierung", {
        method: "POST",
        mode: "no-cors",
        headers: {
    ...
    window.location.href = "http://8kate.github.io/GIS-Abgabe-20-21/sites/verleih.html";
}
